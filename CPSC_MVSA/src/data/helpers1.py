...
def get_GaussianNoisetransforms(severity):
    return transforms.Compose(
        [
            transforms.Resize(256),
            transforms.RandomApply([AddGaussianNoise(variance=severity)], p=0.5),
            transforms.CenterCrop(224),
            transforms.ToTensor(),
            transforms.Normalize(
                mean=[0.46777044, 0.44531429, 0.40661017],
...
def get_SaltNoisetransforms(severity):
    return transforms.Compose(
        [
            transforms.Resize(256),
            transforms.RandomApply([AddSaltPepperNoise(density=severity/100, p=1)], p=0.5),
            transforms.CenterCrop(224),
            transforms.ToTensor(),
            transforms.Normalize(
                mean=[0.46777044, 0.44531429, 0.40661017],
...
def get_vocab(args):
    vocab = Vocab()
    if args.model in ["bert", "mmbt", "concatbert","latefusion",'tmc','cadp_latefusion']:
...
def collate_fn(batch, args):
    ...
    img_tensor = None
    if args.model in ["img", "concatbow", "concatbert", "mmbt","latefusion",'tmc','cadp_latefusion']:
...
def get_data_loaders(args):
    ...
    return train_loader, val_loader, cp_loader, test