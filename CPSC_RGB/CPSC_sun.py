...
from models.dynamic_Classifier_sun import Classifier
...
def get_args(parser):
    ...
    parser.add_argument("--data_path", type=str, default="/sun_rgbd")
    ...
    parser.add_argument("--savedir", type=str, default="./savepath/sun_rgbd")
            ...
            for variant_idx in range(10):
                ...
                for i, pred_label in enumerate(tgt):
                    if pred_label.item() in cp_set_depth[i]:
                        sorted_classes = torch.argsort(pretrain_probs_depth[i], descending=True) # Bug in your original code: argsort used pretrain_probs_rgb instead of pretrain_probs_depth
                        rank = (sorted_classes == pred_label).nonzero().item()
                        reliability = 1 / (rank + 1)
                    else:
                        reliability = 0.0
...
def main():
    ...
    args.name = f"cpsc_sunrgb_eta{args.eta}_sigma{args.sigma}"
