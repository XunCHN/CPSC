...
class EfficientHighOrderGenerator(nn.Module):
    def __init__(self, input_dim, num_variants=10, hidden_dim=512):  # 增加隐藏层维度
...
class JSInformationBottleneck(nn.Module):
    ...
    def forward(self, original_features, high_order_variants):
        ...
        # 3. 组合损失
        ib_loss = self.alpha * consistency_loss 
...
class Classifier(nn.Module):
    def __init__(self, args):
        ...
        # 将单模态表示层扩展到256维（原来的两倍）
        self.unimodal_transform = nn.Sequential(
            nn.Linear(depth_last_size, 256),  # 128 -> 256
            nn.ReLU(),
            nn.Dropout(args.dropout)
        )
        
        # 更新分类层输入维度
        self.rgb_clf = nn.Linear(256, args.n_classes)  # 128 -> 256
        self.depth_clf = nn.Linear(256, args.n_classes)  # 128 -> 256
